...
namespace App\Integrations\Casino\BetPlanet;
...
class CasinoCore implements CasinoCoreInterface
{
    ...
    public function withdraw($userId, $withdrawAmount, $currency, $gameTransactionRef, $gameTransactionUniqueRef = null, $userIP = null, $gameId = null, $additional_data = null)
    {
        ...
        //Doesn't have enought balance to make withdraw
            ...
            return $this->respondError(
                GISErrorCodes::NOT_SUFFICIENT_FUNDS,
                'User doesn\'t have enough balance to withdraw money(bet)',
    ...
    public function rollback($transactionId, $isCasinoTransaction = 0, $statusNote = null, $userId = null, $gameId = null)
    {
        ...
        if (in_array($rollbackResponse['result'], [ErrorCodes::STATUS_SUCCESS, ErrorCodes::TRANSACTION_STATUS_SUCCESS, ErrorCodes::TRANSACTION_STATUS_ROLLBACK])) {
            return $this->respondSuccess([], 'Rollback Was Successful', $rollbackResponse['result']);
        }

        else if($rollbackResponse['result'] == ErrorCodes::TRANSACTION_NOT_FOUND) {
            return $this->respondSuccess([], 'Rollback Was Successful, Transaction was not found', GISErrorCodes::TRANSACTION_NOT_FOUND);
        }
...
//            Log::warn('Getting Balance response was wrong', ['userId' => $userId, 'currencyId' => $currencyId, 'response' => $balanceResponse]);
    ...
    public function withdrawAndDeposit($userId, $withdrawAmount, $depositAmount, $currency, $gameTransactionRef, $gameTransactionUniqueRef = null, $userIP = null, $gameId = null, $additional_data = null)
    {
        ...
        //Send Withdraw Money Request To Casino Core Wallet System
        $withdrawResponse = $this->withdraw($userId,$withdrawAmount,$currency,"bet_".$gameTransactionRef,"bet_".$gameTransactionUniqueRef,$userIP,$gameId,$additional_data);

        //If Withdraw Was Successful Continue noramal flow
        if($withdrawResponse['code'] == GISErrorCodes::SUCCESS) {

            //Save Casino Wallet Generated UniqueID for withdraw in local variable
            $withdrawCasinoTransactionId = $withdrawResponse['data']['casinoTransactionId'];


            //Send Deposit request to caino wallet system
            $depositResponse = $this->deposit($userId,$depositAmount,$currency,"win_".$gameTransactionRef,"win_".$gameTransactionUniqueRef,$userIP,$gameId,$additional_data);

            //If Also Deposit Was Successful
            if($depositResponse['code'] == GISErrorCodes::SUCCESS) {

                //Get unique id generated by casino wallet for deposit transaction
                $depositCasinoTransactionId = $depositResponse['data']['casinoTransactionId'];

                //Notify game vendor that transaction was successful
                return $this->respondSuccess([
                    'casinoTransactionId'=>$withdrawCasinoTransactionId.'_'.$depositCasinoTransactionId
                ]);
            }

            //If Deposit Failed We Send Withdraw Rollback
            else {

                $rollbackWithdrawResponse = $this->rollback("bet_".$gameTransactionRef,0);
                if($rollbackWithdrawResponse['code'] == GISErrorCodes::SUCCESS) {
                    $this->respondError(GISErrorCodes::TRANSACTION_STATUS_REJECTED,'Transaction ws not precessed');
                }
                else {
                    //TODO Log Critical Error And Notify Someone
                    $this->respondError(GISErrorCodes::TRANSACTION_STATUS_REJECTED,'Transaction ws not precessed and withdraw rollback failed');
                }
            ...
            return $this->respondError(
                $withdrawResponse['error_code'],
                "Can not withdraw Money From User Balance"
            );
        }
