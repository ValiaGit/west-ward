...
class MysqliDb
{
    ...
    public function __construct($host = NULL, $username = NULL, $password = NULL, $db = NULL, $port = NULL, $socket = NULL)
    {
        ...
        $this->db = $db;
        if($port == NULL)
            $this->port = ini_get ('mysqli.default_port') || 3306;
        else
            $this->port = $port;
...
//        echo "---";
        ...
        $this->_mysqli = new mysqli($this->host, $this->username, $this->password, $this->db, $this->port, $this->socket)
    ...
    public function getSQLIInstance() {
        return $this->_mysqli;
    }
    ...
    {
        return $this->_mysqli->real_escape_string(trim($str));
    }
    ...
    protected function _buildTableData ($tableData) {
        ...
        foreach ($tableData as $column => $value) {
            ...
            // Subquery value
            if (is_object ($value)) {
                $this->_query .= $this->_buildPair ($column, $value) . ", ";
                continue;
            }
